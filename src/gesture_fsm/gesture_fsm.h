...
// 作用：手势有限状态机（v2.3 食指弯曲控制方案核心模块）。
//
// v2.3 交互模型（点击/移动完全正交，杜绝误判）：
//   IDLE       空闲态：唯一手势 = 开掌长按锁定
//     └─开掌持续≥lock_hold_ms──→ LOCKED
//   LOCKED     锁定运行态：
//     ├─ 开掌长按≥lock_hold_ms──→ IDLE（解锁）
//     ├─ 食指伸直（未弯曲）──→ 每帧 kPointerMove（相对位移移动鼠标）
//     ├─ 快速弯曲→伸直 ──→ kClick（左键单击）
...
//   2. 弯曲判定用"食指自身折叠程度"（食指尖(8)到食指根(5) 距离 /
//      食指根(5)到手腕(0) 距离的比值）——伸直时比值 ≈1.0~1.5，弯曲点击时
//      食指尖折向掌心、比值骤降。与手平移完全正交：平移时食指形态不变，
//      移动永不误触发点击；手指并拢时食指仍伸直、比值不变 → 不会误判。
...
// 手势事件枚举：状态机对外输出的识别结果（v2.3 食指弯曲方案）
...
struct FsmConfig {
    ...
    // 食指弯曲判定（点击/拖拽动作，v2.3 核心）
    float bendRatio = 0.8f;             // 弯曲比值阈值：食指尖(8)到食指根(5) / 食指根(5)到手腕(0)
                                        // < 此值 = 弯曲。用"食指自身折叠程度"判定，
                                        // 手指并拢（食指仍伸直）不会误判
