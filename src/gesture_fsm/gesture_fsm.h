...
// 作用：手势有限状态机（v2.4 握拳点击控制方案核心模块）。
//
// v2.4 交互模型（点击/移动天然互斥，杜绝误判）：
//   IDLE       空闲态：唯一手势 = 开掌长按锁定
//     └─开掌持续≥lock_hold_ms──→ LOCKED
//   LOCKED     锁定运行态：
//     ├─ 开掌长按≥lock_hold_ms──→ IDLE（解锁）
//     ├─ 食指伸直（非握拳）──→ 每帧 kPointerMove（掌心相对位移移动鼠标）
//     ├─ 快速握拳→松开 ──→ kClick（左键单击）
//     ├─ 握拳保持 ≥fist_hold_ms ──→ kDragStart（拖拽开始，左键按住）
//     ├─ 拖拽中移动 ──→ kDragMove（左键保持 + 跟手）
//     └─ 松开握拳 ──→ kDragEnd（拖拽结束，释放左键）
//
// 为何用"握拳"（v2.3 弯曲/捏合方案的问题）：
//   1. 捏合、食指弯曲等"局部手指动作"都与日常手部状态有模糊地带（易误判）。
//   2. 握拳判定 = 所有 5 指尖到手腕距离均 < 阈值；移动判定 = 食指伸出
//      （食指尖到手腕距离 > 阈值）。两者必然互斥：握拳时食指尖也收拢、
//      移动时食指伸直，绝不可能同时满足 —— 这是最可靠的"点击 vs 移动"区分。
//   3. 握拳动作幅度大、特征显著，手部自然状态（半握/并拢）不会触发。
//   4. v1 时代握拳短按本来就可用，只是当时被锁定手势占用；现在开掌接管锁定，
//      握拳完全腾出给点击/拖拽。
//
// 事件输出：
...
// 手势事件枚举：状态机对外输出的识别结果（v2.4 握拳方案）
enum class GestureEvent {
    kNone         = 0,   // 无动作
    kOpenPalmHold = 1,   // 开掌长按：锁定/解锁开关（≥lock_hold_ms）
    kPointerMove  = 2,   // 掌心定位移动（LOCKED 态非握拳时每帧上报）
    kClick        = 3,   // 快速握拳→松开：左键单击
    kDragStart    = 4,   // 握拳保持（超 fist_hold_ms）：拖拽开始
    kDragMove     = 5,   // 拖拽中移动（左键保持按下）
    kDragEnd      = 6,   // 松开握拳：拖拽结束（释放左键）
};
...
struct FsmConfig {
    ...
    // 握拳判定（点击/拖拽动作，v2.4 核心）
    int   fistDistThresholdPx = 160;    // 握拳距离阈值（像素）：所有 5 指尖(4,8,12,16,20)到手腕(0)
                                        // 距离均 < 此值 = 握拳。与"食指伸出移动"天然互斥
    int   fistHoldMs = 350;             // 握拳保持时长阈值（毫秒）：保持超过 = 拖拽开始；之前松开 = 单击
...
private:
    ...
    // 握拳/拖拽检测状态（v2.4 核心）
    bool  m_fisting = false;                // 是否处于握拳中（等待区分单击/拖拽）
    ...
    // 握拳判定：所有 5 指尖(4,8,12,16,20)到手腕(0)距离均 < fistDistThresholdPx
    bool detectFist(const HandKeypoints& kp);
