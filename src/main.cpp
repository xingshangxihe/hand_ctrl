...
int main(int argc, char* argv[]) {
    ...
    while (!g_shouldExit.load()) {
        ...
        // 注意：只检测"无新帧"，【不要】用"palm 持续无效"判花屏——
        //   手不在画面时 palm 检测不到是【正常现象】，若据此重开会反复误触
        //   （实测：什么都不做时画面反复卡住）。真实 MJPG 损坏最终都会体现为
        //   g_lastFrameUs 停止更新（解码失败→队列空→无新帧），由本看门狗覆盖。
        {
