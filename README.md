...
> 当前版本：**v2.4（握拳点击 + 相对位移方案）**——开掌锁定，掌心相对位移控制鼠标，握拳点击/拖拽。
>
> v1.0（六手势方案）已废弃：握拳短按、OK、竖拇指、食指滑动等姿态手势。
> v2.4 保留：开掌锁定/解锁 + 掌心相对位移 + 握拳点击/拖拽。
...
## v2.4 交互模型（唯一基准，全部阈值外置 `config/gesture_config.json`）

**核心思想：点击（握拳）与移动（食指伸出）天然互斥，移动采用相对位移（触控板式）。**

| 动作 | 判定规则 | 输出事件 |
| ---- | -------- | -------- |
| 五指张开长按 1s | 5 根手指全部伸直（指尖/指根比值 > 阈值）持续 1s | 锁定/解锁切换（IDLE↔LOCKED） |
| 手移动 | 非握拳（食指伸出），掌心帧间位移 | 鼠标相对位移（触控板式，可覆盖整个屏幕） |
| 快速握拳→松开 | 所有 5 指尖到手腕距离 < 阈值后松开 | 左键单击 |
| 握拳保持 | 握拳保持超过 `fist_hold_ms` | 左键按住（拖拽开始） |
| 拖拽中移动 | 左键按住时手移动 | 鼠标跟手移动（拖拽） |
| 松开握拳 | 握拳状态解除 | 释放左键（拖拽结束） |

> **手势设计说明**：
> - 锁定/解锁用「五指张开」而非「握拳」——握拳留给点击/拖拽（v1 时代握拳被锁定
>   手势占用，现在腾出）。开掌（5 指全伸直）与握拳区分度极高。判定用"指尖到
>   手腕 / 指根到手腕"比值法，对"手指朝屏幕"的透视投影缩短鲁棒。
> - 点击/拖拽用「握拳」——握拳判定 = 所有 5 指尖到手腕距离均 < 阈值；移动判定 =
>   食指伸出（食指尖距离 > 阈值）。**两者必然互斥**：握拳时食指尖也收拢、移动时
>   食指伸直，绝不可能同时满足 → 这是最可靠的"点击 vs 移动"区分，杜绝误判。
>   握拳动作幅度大、特征显著，手部自然状态（半握/并拢）不会触发。
...
2. **移动鼠标**：张开手（或伸出食指），手移动 → 鼠标相对移动（触控板式，手可小范围来回划动，鼠标能到屏幕任意位置）
3. **单击**：快速握拳再松开（如点击屏幕上按钮）
4. **拖拽**：握拳保持不放并移动 → 可拖动窗口/文件；松开即释放
...
- `lock.*`/`open_palm.*`/`fist.*` 阈值 → FSM 立即采用新值
...
- [x] **握拳点击/拖拽**：5 指尖到手腕距离判定，与"食指伸出移动"天然互斥，杜绝误判
