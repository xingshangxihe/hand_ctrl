...
> 当前版本：**v2.1（捏合控制方案）**——开掌锁定，食指定位，捏合点击/拖拽。
>
> v1.0（六手势方案）已废弃：握拳短按、OK、竖拇指、食指滑动、握拳拖拽等
> 姿态手势全部移除。v2.1 只保留：开掌锁定/解锁 + 食指定位 + 捏合点击/拖拽。
...
## v2.1 交互模型（唯一基准，全部阈值外置 `config/gesture_config.json`）

**核心思想：点击（捏合）与移动（平移）完全正交，彻底杜绝误判。**

| 动作 | 判定规则 | 输出事件 |
| ---- | -------- | -------- |
| 五指张开长按 1.2s | 5 根手指全部伸直（指尖/指根比值 > 阈值）持续 1.2s | 锁定/解锁切换（IDLE↔LOCKED） |
| 食指移动 | 拇指食指张开（未捏合），食指尖每帧上报 | 鼠标绝对定位（指哪打哪） |
| 快速捏合→松开 | 拇指尖(4)与食指尖(8)距离 < 阈值后松开 | 左键单击 |
| 捏合保持 | 捏合保持超过 `pinch_hold_ms` | 左键按住（拖拽开始） |
| 拖拽中移动 | 左键按住时食指移动 | 鼠标跟手移动（拖拽） |
| 松开捏合 | 捏合状态解除 | 释放左键（拖拽结束） |

> **手势设计说明**：
> - 锁定/解锁用「五指张开」而非「握拳」——单指控制时"食指伸直+其余弯曲"与握拳
>   在摄像头视角下难以区分；开掌（5 指全伸直）区分度极高。判定用"指尖到手腕 /
>   指根到手腕"比值法，对"手指朝屏幕"的透视投影缩短鲁棒。
> - 点击用「捏合」而非「下点」——v2.0 用食指尖 y 方向速度判下点，与移动天然冲突
>   （移动时手指本身有 y 分量）。捏合判定只看拇指尖与食指尖距离，与手平移完全
>   正交：平移时捏合状态不变，移动永不误触发点击。
...
1. **锁定**：对摄像头张开五指并保持 1.2s → 状态变 `LOCKED`
2. **移动鼠标**：拇指食指张开（不要捏合），食指指尖位置 = 鼠标指针（指哪打哪）
3. **单击**：拇指+食指快速捏合一下再松开（如点击屏幕上按钮）
4. **拖拽**：捏合保持不放并移动 → 可拖动窗口/文件；松开即释放
5. **解锁**：再次张开五指保持 1.2s → 状态回 `IDLE`

### 配置热加载（实时生效）

运行中直接编辑 `config/gesture_config.json`，约 1 秒后自动生效：
- `lock.*`/`open_palm.*`/`pinch.*` 阈值 → FSM 立即采用新值
...
- [x] **移动直线修正**：位移 EMA 低通滤波 + 死区，抑制手部高频抖动，路径更直更稳
- [x] **捏合点击/拖拽**：拇指+食指捏合判定单击/拖拽，与移动完全正交（替代全部姿态手势）
