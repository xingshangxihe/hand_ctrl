...
> 当前版本：**v2.3（食指弯曲 + 相对位移方案）**——开掌锁定，食指相对位移控制鼠标，弯曲点击/拖拽。
>
> v1.0（六手势方案）已废弃：握拳短按、OK、竖拇指、食指滑动、握拳拖拽等
> 姿态手势全部移除。v2.3 只保留：开掌锁定/解锁 + 食指相对位移 + 食指弯曲点击/拖拽。
...
## v2.3 交互模型（唯一基准，全部阈值外置 `config/gesture_config.json`）

**核心思想：点击（食指弯曲）与移动（平移）完全正交，移动采用相对位移（触控板式）。**

| 动作 | 判定规则 | 输出事件 |
| ---- | -------- | -------- |
| 五指张开长按 1s | 5 根手指全部伸直（指尖/指根比值 > 阈值）持续 1s | 锁定/解锁切换（IDLE↔LOCKED） |
| 食指移动 | 食指伸直（未弯曲），食指尖帧间位移 | 鼠标相对位移（触控板式，可覆盖整个屏幕） |
| 快速弯曲→伸直 | 食指尖(8)/食指根(5) 与食指根(5)/手腕(0) 比值 < 阈值后伸直 | 左键单击 |
| 弯曲保持 | 弯曲保持超过 `bend_hold_ms` | 左键按住（拖拽开始） |
| 拖拽中移动 | 左键按住时食指移动 | 鼠标跟手移动（拖拽） |
| 食指伸直 | 弯曲状态解除 | 释放左键（拖拽结束） |

> **手势设计说明**：
> - 锁定/解锁用「五指张开」而非「握拳」——单指控制时"食指伸直+其余弯曲"与握拳
>   在摄像头视角下难以区分；开掌（5 指全伸直）区分度极高。判定用"指尖到手腕 /
>   指根到手腕"比值法，对"手指朝屏幕"的透视投影缩短鲁棒。
> - 点击用「食指弯曲」而非「捏合/下点」——捏合（拇指+食指闭合）动作不自然，
>   下点（y 速度）与移动天然冲突。食指弯曲类似按鼠标键，直觉自然且与移动正交。
>   判定用"食指自身折叠程度"（食指尖到食指根 / 食指根到手腕的比值）：
>   手指并拢（其他手指靠近食指）时食指仍伸直、比值不变 → 不会误判为弯曲。
> - 移动用「相对位移」（触控板式）而非绝对定位——绝对定位要求手覆盖整个画面
>   才能覆盖整个屏幕，但手活动范围有限做不到；相对位移可累计，手在小范围内
>   来回移动多次即可让鼠标到达屏幕任意位置。
...
- **开掌锁定防误触**：张开五指 1.2s 锁定后控制才生效，大幅降低误触率
...
2. **移动鼠标**：食指伸直，手移动 → 鼠标相对移动（触控板式，手可小范围来回划动，鼠标能到屏幕任意位置）
3. **单击**：食指快速弯曲一下再伸直（如点击屏幕上按钮，类似按鼠标键）
4. **拖拽**：食指弯曲保持不放并移动 → 可拖动窗口/文件；伸直即释放
5. **解锁**：再次张开五指保持 1s → 状态回 `IDLE`
...
- `lock.*`/`open_palm.*`/`click.*` 阈值 → FSM 立即采用新值
...
- [x] **移动直线修正**：指尖坐标 EMA 低通滤波 + 死区，抑制手部高频抖动，路径更直更稳
- [x] **食指弯曲点击/拖拽**：食指自身折叠比值判定（并拢不误判），与移动完全正交
- [x] **拖拽画面隐藏**：拖拽时自动隐藏画面窗口（避免虚拟鼠标触发 GTK 窗口交互），结束立即恢复
- [x] **开掌锁定/解锁**：唯一保留的姿态手势（5 指全伸直比值判定），1.2s 防误触
