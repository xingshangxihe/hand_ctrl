...
> 姿态手势全部移除。v2 只保留：开掌锁定/解锁 + 食指定位/点击/拖拽。
...
| 食指移动 | 非开掌状态，食指指尖每帧上报 | 鼠标绝对定位（指哪打哪） |
...
> **手势设计说明**：锁定/解锁用「五指张开」而非「握拳」——单指控制时手指常处于
> "食指伸直+其余弯曲"状态，与握拳在摄像头视角下难以区分；而开掌（5 指全伸直）
> 与单指控制区分度极高，不会误触。判定用"指尖到手腕 / 指根到手腕"比值法，
> 对"手指朝屏幕"的透视投影缩短鲁棒。
...
2. **移动鼠标**：收起其余手指只伸食指，食指指尖位置 = 鼠标指针（指哪打哪）
