...
> 当前版本：**v2.2（食指弯曲控制方案）**——开掌锁定，食指定位，弯曲点击/拖拽。
>
> v1.0（六手势方案）已废弃：握拳短按、OK、竖拇指、食指滑动、握拳拖拽等
> 姿态手势全部移除。v2.2 只保留：开掌锁定/解锁 + 食指定位 + 食指弯曲点击/拖拽。
...
## v2.2 交互模型（唯一基准，全部阈值外置 `config/gesture_config.json`）

**核心思想：点击（食指弯曲）与移动（平移）完全正交，彻底杜绝误判。**

| 动作 | 判定规则 | 输出事件 |
| ---- | -------- | -------- |
| 五指张开长按 1s | 5 根手指全部伸直（指尖/指根比值 > 阈值）持续 1s | 锁定/解锁切换（IDLE↔LOCKED） |
| 食指移动 | 食指伸直（未弯曲），食指尖每帧上报 | 鼠标绝对定位（指哪打哪） |
| 快速弯曲→伸直 | 食指尖(8)到中指根(9)距离 < 阈值后伸直 | 左键单击 |
...
> - 点击用「食指弯曲」而非「捏合」——捏合（拇指+食指闭合）动作不自然，长时间
>   操作累。食指弯曲类似按鼠标键（食指尖折向中指根），直觉自然，且与手平移完全
>   正交：平移时两指相对位置不变，移动永不误触发点击。
...
- `screen.screen_width/height`、`mouse.scale_x/scale_y` → 绝对定位映射立即更新
...
- [x] **食指弯曲点击/拖拽**：食指尖折向中指根判定单击/拖拽（类似按鼠标键），与移动完全正交
