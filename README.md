...
> 当前版本：**v2.5（捏合单击 + 双指V拖拽方案）**——开掌锁定，掌心相对位移控制鼠标，捏合单击，双指V拖拽。
>
> v1.0（六手势方案）已废弃：握拳短按、OK、竖拇指、食指滑动等姿态手势。
> v2.5 保留：开掌锁定/解锁 + 掌心相对位移 + 捏合单击 + 双指V拖拽。
...
## v2.5 交互模型（唯一基准，全部阈值外置 `config/gesture_config.json`）

**核心思想：单击（捏合）、拖拽（双指V）、移动（食指伸出）三个动作完全分离。**

| 动作 | 判定规则 | 输出事件 |
| ---- | -------- | -------- |
| 五指张开长按 1s | 5 根手指全部伸直（指尖/指根比值 > 阈值）持续 1s | 锁定/解锁切换（IDLE↔LOCKED） |
| 手移动 | 食指伸出（非捏合/非双指V），掌心帧间位移 | 鼠标相对位移（触控板式，可覆盖整个屏幕） |
| 捏合 tap | 拇指尖(4)-食指尖(8)距离 ÷ 中指根(9)-手腕(0)距离 < 阈值（比例法，对距离鲁棒） | 左键单击 |
...
- [x] **捏合单击 + 双指V拖拽**：两个动作完全分离；捏合用比例判定（对距离鲁棒）
